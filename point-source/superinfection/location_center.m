% i starts at the center, j varies -- superinfection
...
saveas(figI, fullfile(output_folder, 'location1_I_center_v2'));
close(figI);

figure(figJ); xlabel('Time'); ylabel('J-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figJ, fullfile(output_folder, 'location1_J_center_v2'));
close(figJ);
