% i starts at the corner, j varies -- superinfection
...
all_dist=[1*40+2,9*40+10,1190,1600];
% 2, 18, 58, 78
...
num_t=300; time_bet=1;

for d_idx=1:length(all_dist)
    
    d=all_dist(d_idx);

    S0=ones(N, 1); I0=zeros(N, 1); J0=zeros(N, 1);
    I0(1)=0.05; J0(d)=0.05;
...
saveas(figI, fullfile(output_folder, 'location1_I_corner'));
close(figI);

figure(figJ); xlabel('Time'); ylabel('J-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figJ, fullfile(output_folder, 'location1_J_corner'));
close(figJ);
