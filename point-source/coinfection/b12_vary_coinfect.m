...
for d_idx=1:length(all_b12)
    ...
    I0_mono(1)=0.05; J0_mono(1600)=0.05;

    y0=[S0;I0;J0;C0];

    nodes_I=zeros(1, num_t+1); nodes_I_mono=zeros(1, num_t+1);
    nodes_J=zeros(1, num_t+1); nodes_J_mono=zeros(1, num_t+1);
...
saveas(figI, fullfile(output_folder, 'b12_I_v2'));
close(figI);

figure(figJ); xlabel('Time'); ylabel('J-Spread Index');
legend(compose('b_{12} = %.3f', all_b12), 'Location', 'best'); grid on;
saveas(figJ, fullfile(output_folder, 'b12_J_v2'));
close(figJ);

figure(figC); xlabel('Time'); ylabel('Co-Spread Index');
legend(compose('b_{12} = %.3f', all_b12), 'Location', 'best'); grid on;
saveas(figC, fullfile(output_folder, 'b12_C_v2'));
close(figC);

figure(figI_mono); xlabel('Time'); ylabel('I_1-Spread Index');
legend(compose('b_{12} = %.3f', all_b12), 'Location', 'best'); grid on;
saveas(figI_mono, fullfile(output_folder, 'b12_I_mono_v2'));
close(figI_mono);

figure(figJ_mono); xlabel('Time'); ylabel('I_2-Spread Index');
legend(compose('b_{12} = %.3f', all_b12), 'Location', 'best'); grid on;
saveas(figJ_mono, fullfile(output_folder, 'b12_J_mono_v2'));
close(figJ_mono);
