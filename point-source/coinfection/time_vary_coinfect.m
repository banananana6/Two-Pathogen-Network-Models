...
saveas(figI, fullfile(output_folder, 'time_I'));
close(figI);

figure(figJ); xlabel('Time'); ylabel('J-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figJ, fullfile(output_folder, 'time_J'));
close(figJ);

figure(figC); xlabel('Time'); ylabel('C-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figC, fullfile(output_folder, 'time_C'));
close(figC);

figure(figI_mono); xlabel('Time'); ylabel('I_1-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figI_mono, fullfile(output_folder, 'time_I_mono'));
close(figI_mono);

figure(figJ_mono); xlabel('Time'); ylabel('I_2-Spread Index');
legend(compose('d_{ij} = %.3f', all_dist), 'Location', 'best'); grid on;
saveas(figJ_mono, fullfile(output_folder, 'time_J_mono'));
close(figJ_mono);
