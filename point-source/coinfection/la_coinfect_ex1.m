...
legend(compose('(LA%d,LA%d,LA%d)', [all_k1(:), all_k2(:), all_k3(:)]), 'Location', 'best');
saveas(figC, fullfile(output_folder, 'la_C_v1'));
close(figC);

figure(figI_mono); xlabel('Time'); ylabel('I_1-Spread Index');
legend(compose('(LA%d,LA%d,LA%d)', [all_k1(:), all_k2(:), all_k3(:)]), 'Location', 'best');
saveas(figI_mono, fullfile(output_folder, 'la_I_mono_v1'));
close(figI_mono);

figure(figJ_mono); xlabel('Time'); ylabel('I_2-Spread Index');
legend(compose('(LA%d,LA%d,LA%d)', [all_k1(:), all_k2(:), all_k3(:)]), 'Location', 'best');
saveas(figJ_mono, fullfile(output_folder, 'la_J_mono_v1'));
close(figJ_mono);
